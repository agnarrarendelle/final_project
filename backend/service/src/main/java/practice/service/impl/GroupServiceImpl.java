...
public class GroupServiceImpl implements GroupService {
    ...
    public List<GroupVo> getGroups(Integer userId) {
        UserEntity user = userRepository.findByIdWithGroups(userId).orElseThrow(
                () -> new UserNotFoundException(userId)
        );
    ...
    public UserVo addNewUser(Integer userId, Integer groupId) {
        GroupEntity group = groupRepository.findByIdWithUsers(groupId).orElseThrow(
                () -> new GroupNotFoundException(groupId)
        );
        UserEntity newUser = userRepository.findById(userId).orElseThrow(
                () -> new UserNotFoundException(userId)
        );

        userGroupRepository.findByUserIdAndGroupId(userId, groupId).ifPresent(
                (s) -> {
