...
public class GlobalExceptionHandler {
    ...
    @ResponseStatus(HttpStatus.BAD_GATEWAY)
    public Result<Void> exception(UserNotFoundException exception) {
        return Result.error(exception.getMessage());
    }

    @ExceptionHandler(GroupNotFoundException.class)
    @ResponseStatus(HttpStatus.BAD_GATEWAY)
    public Result<Void> exception(GroupNotFoundException exception) {
        return Result.error(exception.getMessage());
    }

    @ExceptionHandler(UserAlreadyInGroupException.class)
    @ResponseStatus(HttpStatus.BAD_GATEWAY)
    public Result<Void> exception(UserAlreadyInGroupException exception) {
        return Result.error(exception.getMessage());
    }

    @ExceptionHandler(CategoryNotFoundException.class)
    @ResponseStatus(HttpStatus.BAD_GATEWAY)
    public Result<Void> exception(CategoryNotFoundException exception) {
        return Result.error(exception.getMessage());
    }
}
